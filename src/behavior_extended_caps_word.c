 ...
 *
 * Extended caps word: stock caps_word plus QMK-style MINUS → UNDERSCORE.
 */
...
static bool caps_word_should_shift(uint8_t usage_id) {
    return caps_word_is_alpha(usage_id) ||
           usage_id == HID_USAGE_KEY_KEYBOARD_MINUS_AND_UNDERSCORE;
}

static void caps_word_enhance_usage(const struct behavior_caps_word_config *config,
                                    struct zmk_keycode_state_changed *ev) {
    if (ev->usage_page != HID_USAGE_KEY || !caps_word_should_shift(ev->keycode)) {
        return;
    }
...
#define KP_INST(n)                                                                                 \
    static struct behavior_caps_word_data behavior_extended_caps_word_data_##n = {.active = false}; \
    static const struct behavior_caps_word_config behavior_extended_caps_word_config_##n = {        \
